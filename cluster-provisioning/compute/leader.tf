...
#   }
#   provisioner "local-exec" {
#     command = "scp -i ${var.ssh_key_path} -o StrictHostKeyChecking=accept-new ${local.vm_user}@${var.cluster_public_ip}:~/.kube/config .terraform/.kube/config-cluster"
#   }
#   provisioner "local-exec" {
#     command = "scp -i ${var.ssh_key_path} -o StrictHostKeyChecking=accept-new ${local.vm_user}@${var.cluster_public_ip}:~/.kube/config-external .terraform/.kube/config-external"
#   }
#   provisioner "local-exec" {
